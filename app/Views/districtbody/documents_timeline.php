...
$(window).click(function(e) {
	...
	if($(e.target).hasClass('submit_filter'))
	{
		...
		if(school == "") { alert("Please select the School."); }
		else if(year == "") { alert("Please select the Year."); }
		else{
			$.ajax({
				url:"<?php echo BASE_URL.'district/get_documents_timeline'; ?>",
				type:"post",
				dataType:"json",
				data:{school:school,year:year},
	...
	if($(e.target).hasClass('download_full_report'))
    {
      ...
      setTimeout( function() {
        var template_id = $(e.target).attr("data-element");
          ...
          $.ajax({
            url:"<?php echo BASE_URL.'district/download_report_pdf'; ?>",
            type:"post",
            ...
            success:function(result)
            {
              ...
              //SaveToDiskdownload(base_url+'papers/district/'+result,result);
                ...
                var src = 'papers/district/'+result;

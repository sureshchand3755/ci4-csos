...
$(window).change(function(e) {
	if($(e.target).hasClass('select_district'))
	{
		...
		$.ajax({
			url:"<?php echo BASE_URL.'school/user_school_lists'; ?>",
			type:"post",
...
$(window).click(function(e) {
	...
	if($(e.target).hasClass('submit_filter'))
	{
		...
		if(district == "") { alert("Please select the District."); }
			...
			$.ajax({
				url:"<?php echo BASE_URL.'school/user_get_documents_timeline'; ?>",
				type:"post",
				dataType:"json",
	...
	if($(e.target).hasClass('download_full_report'))
    {
      ...
      setTimeout( function() {
        var template_id = $(e.target).attr("data-element");
          ...
          $.ajax({
            url:"<?php echo BASE_URL.'school/user_download_report_pdf'; ?>",
            type:"post",
    ...
    if($(e.target).hasClass('download_full_survey'))
    {
      ...
      setTimeout( function() {
        var template_id = $(e.target).attr("data-element");
          var base_url = "<?php echo BASE_URL; ?>";
          $.ajax({
            url:"<?php echo BASE_URL.'school/user_download_pdf'; ?>",
            type:"post",
            data:{template_id:template_id},
            success:function(result)
            {

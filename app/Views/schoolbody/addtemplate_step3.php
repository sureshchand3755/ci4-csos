        ...
        <div class="modal-body">
          ...
          <div class="img_div">
             <div class="image_div_attachments">
               <form action="<?php echo BASE_URL.'school/upload_images'; ?>" method="post" enctype="multipart/form-data" class="dropzone" id="imageUpload" style="clear:both;min-height:80px;background: #949400;color:#000;border:0px solid; height:auto; width:100%; float:left">
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary print_selected_sections">Print</button>
        	    ...
        	    <!-- Small boxes (Stat box) -->
                     <!-- BEGIN FORM-->
                    <form action="<?php echo BASE_URL.'school/save_template_content_step3'; ?>" id="form_sample_3" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="tab-pane active main_table_row" id="home1" role="tabpanel" aria-expanded="false">
                                    	...
                                    	if(!empty($forms))
                                    	{
                                    		...
                                    		foreach($forms as $key => $form)
                                    		{
                                    			...
                                    			if($key == 0)
                                    			{
                                                    ...
                                                    <div class="col-md-4 padding_00">
                                                      <h6 style="text-align:center">Attachment</h6>
                                                    </div>
					                                        </div>
                                                        ...
                                                        <div class="col-md-4 padding_00">
                                                          &nbsp;
							                                        ...
							                                        <div class="col-md-2 action_div">
							                                        	<input type="hidden" name="mark[]" class="hidden_mark" value="0">
							                                        	&nbsp;
							                                        	<textarea name="comments[]" class="form-control comments_input" '.$disabled.' style="display:none">'.$input['comments'].'</textarea>
							                                        </div>
							                                    </div>
							                                    ';
							                                 }
                                                ...
                                                if($template_details['status'] >= 3) { 
                                                  ...
                                                  $url_upload = BASE_URL.'school/addtemplate_step3/'.$input['template_id'];
                                                } 
                                                else{
                                                  $files = $this->db->select('*')->from('template_attachments')->where('form_id',$input['id'])->get()->result_array();
                                                  if(!empty($files))
                                                  ...
                                                  $url_upload = BASE_URL.'school/addtemplate_step3/'.$input['template_id'];
                                                        ...
                                                        if($template_details['status'] >= 3)
                                                        {
                                                          ...
                                                          if($input['attachment'] == 1) {
                                                            $outputval.='<i class="fa fa-plus faplus '.$disabled.'" style="margin-top:10px" aria-hidden="true" data-element="'.BASE_URL.'school/upload_images?file_id='.$input['id'].'" data-value="'.$input['id'].'" title="Add Attachment"></i><br/>';
                                                          }         
                                                        }                                             
                                                        $outputval.=$downloadfile.'</div>
							                                        </div>
							                                        <div class="col-md-2 action_div">
							                                        	<input type="hidden" name="mark[]" class="hidden_mark" value="'.$mark_value.'">
                                                        <input type="hidden" name="sample[]" class="hidden_sample" value="">
							                                        	<textarea name="comments[]" class="form-control comments_input" '.$disabled.'>'.$input['comments'].'</textarea>
							                                        </div>
							                                    </div>
							                                    ';
							                                }
					                            ...
					                            if($template_details['status'] == 4) { $hide_summary = 'display:block'; }
					                            else{ $hide_summary = 'display:none'; }
					                            $outputval.='</div>
					                                <div class="col-md-8 summary_div" style="'.$hide_summary.'">
                                          ...
                                          <div class="col-md-4" style="'.$hide_summary.'">';
                                              ...
                                              $outputval.='<label class="summary_label">Attachments:</label><br/>';
                        ...
                        if($template_details['status'] == 3) {
                        	?>
                          <a href="<?php echo BASE_URL.'school/manage_submitted_surveys'; ?>" class="btn btn-primary" style="margin-top:20px">Go Back to Submitted Survey</a>
                        	<?php
                        }
                        elseif($template_details['status'] == 4) {
                          ?>
                          <a href="<?php echo BASE_URL.'school/manage_reviewed_surveys'; ?>" class="btn btn-primary" style="margin-top:20px">Go Back to Reviewed Survey</a>
                          <?php
                        }
                        else { ?>
                        	<input type="submit" class="btn btn-primary submit_step_3" name="submit_step3" value="Submit completed Survey" style="margin-top:20px">
                        <?php } ?>
                    </form>
...
$(window).click(function(e) {
  ...
  if($(e.target).hasClass('trash_image'))
  {
    ...
    if (r == true) {
      ...
      $.ajax({
          url:"<?php echo BASE_URL.'school/infile_delete_image'; ?>",
          type:"get",
          data:{imgid:imgid},
          success: function(result) {
            window.location.reload();
  ...
  if($(e.target).hasClass('download_selected_sections'))
  {
    ...
    setTimeout( function() {
      ...
      $.ajax({
        url:"<?php echo BASE_URL.'school/download_pdf_sections'; ?>",
        type:"post",
  ...
  if($(e.target).hasClass('print_selected_sections'))
  {
    ...
    setTimeout( function() {
      ...
      var base_url = "<?php echo BASE_URL; ?>";
      $.ajax({
        url:"<?php echo BASE_URL.'school/print_pdf_sections'; ?>",
        type:"post",
        data:{template_id:template_id,sections:sections},
        success:function(result)
        {
	...
	if($(e.target).hasClass('download_pdf'))
    {
    	...
    	setTimeout( function() {
	        ...
	        var base_url = "<?php echo BASE_URL; ?>";
	        $.ajax({
	        	url:"<?php echo BASE_URL.'school/download_pdf'; ?>",
	        	type:"post",
    ...
    if($(e.target).hasClass('print_pdf'))
    {
    	...
    	setTimeout( function() {
	        ...
	        var base_url = "<?php echo BASE_URL; ?>";
	        $.ajax({
	        	url:"<?php echo BASE_URL.'school/print_pdf'; ?>",
	        	type:"post",
	        	data:{template_id:template_id},
	        	success:function(result)
	        	{
    ...
    if($(e.target).hasClass('yes_hit'))
    {
	    ...
	    $.ajax({
	        url:"<?php echo BASE_URL.'school/ajax_create_master_template_step3'; ?>",
	        type:"post",
    ...
    {
      e.preventDefault();
      var mark = $(".hidden_sample").length;
      var count = $(".input_mark:checked").length;
      if(mark == count)
      {
        $(".alert_modal_submit").modal("show");
        $(".alert_content_submit").html("Are you sure you want to submit the survey? Once submitted you will not be able to make changes to the survey.");
      }
    	else{
        $(".alert_modal_no_btns").modal("show");
        $(".alert_content_no_btns").html("You should answer all the questions and then click on the 'Submit Completed Survey' button.");
        $(".hidden_mark").each(function() {
          if($(this).val() == "0")
          {
            if($(this).parents(".inside_table_div").find(".input_mark").length > 0)
            {
              $(this).parents(".inside_table_div").find("h6").css("color", "#f00");
            }
          }
        })
      }
    ...
    $(e.target).parents(".inside_table_div").find("h6").css("color", "#000");
	}
});
...
<?php
if($template_details['status'] >= 3) { } else { ?>
